...
__global__ void CoulombForce(float *ForceX, float *ForceY, float *ForceZ, const float *PosX,const float *PosY,const float *PosZ, int N)
{
	...
	for(int n = 0; n  < idx; n++)
	{
		...
		ForceX[idx] += (PosX[idx]-PosX[n])/dcubed;
		ForceY[idx] += (PosY[idx]-PosY[n])/dcubed;
		ForceZ[idx] += (PosZ[idx]-PosZ[n])/dcubed;
	...
	// Im just trying to get ride of errors.
	 float fPI = acos(-1.0);
...
void FastCoulombWrapper(float * ForceX, float * ForceY, float * ForceZ,const float * PosX,const float * PosY,const float * PosZ, float * ForceX_d, float * ForceY_d, float * ForceZ_d, float * PosX_d, float * PosY_d, float * PosZ_d, const int Nions)
{
	size_t size = Nions * sizeof(float);
	// copying data to device
	cudaMemcpy(PosX_d, PosX, size, cudaMemcpyHostToDevice);
...
void CudaCoulombAlloc(float ** ForceX_d, float ** ForceY_d, float ** ForceZ_d, float ** PosX_d, float ** PosY_d, float ** PosZ_d, const int Nions)
{
...
void CudaCoulombFree(float * ForceX_d, float * ForceY_d, float * ForceZ_d, float * PosX_d, float * PosY_d, float * PosZ_d)
{
	cudaFree(PosX_d);
	cudaFree(PosY_d);
	cudaFree(PosZ_d);
